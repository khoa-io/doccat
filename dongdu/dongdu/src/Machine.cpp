...
namespace dongdu {
	...
	string Machine::TAG = "[dongdu::Machine] ";
...
void Machine::init(int window_length, string path, StrMapReference ref)
{
...
void Machine::convert(string sentence)
{
	...
	// - xuyenhb - while (sentence[0] <= 40 && sentence.length() > 0) sentence.erase(0);

	size_t length = sentence.length();
	// xuyenhb - while (sentence[length - 1] <= 40 && length > 0) sentence.erase(--length);
...
bool Machine::load()
{
	cout << TAG << "Start loading ..." << endl;
	string modelFilePath = PATH + SEPARATOR_CHAR + "dongdu.model";
	string mapFilePath = PATH + SEPARATOR_CHAR + "dongdu.map";
	_model = load_model(modelFilePath.c_str());
	if (!_model)
	{
		cout << TAG << "Failed to load " << modelFilePath << endl;
		return false;
	}
	cout << TAG << "Loaded " << modelFilePath << endl;

	if (!strmap.load(mapFilePath.c_str())){
		cout << TAG << "Failed to load " << mapFilePath << endl;
		return false;
	}
	cout << TAG << "Loaded " << mapFilePath << endl;
	cout << TAG << "Loading has been completed!" << endl;
	return true;
}
...
string Machine::segment(string sentence)
{
	...
	// - xuyenhb - while (ans[0] <= 40 && ans.length() > 0) ans.erase(0);
	string dummy = "";
	dummy += ans[0];
